...
OnvifSoap.prototype.requestCommand = function(oxaddr, methodName, soap) {
    return this._request(oxaddr, soap, methodName)
        ...
        .then(response => {
            let parsed = this._parseResponseResult(methodName, response);
            if (!parsed) {
...
OnvifSoap.prototype._parseResponseResult = function(methodName, response) {
    if (!('Body' in response)) {
        return null;
    }

    const responseKey = `${methodName}Response`;
    if (!(responseKey in response['Body'])) {
        return null;
    }

    return response['Body'][responseKey];
};
