...
OnvifSoap.prototype.requestCommand = function(oxaddr, methodName, soap) {
    return this._request(oxaddr, soap)
        .then(xml => this.parse(xml))
        .then(result => {
            ...
            let parsed = this._parseResponseResult(methodName, result);
            if (!parsed) {
            	throw new Error(`The device does not seem to support the ${methodName}() method.`);
            }

            return Promise.resolve(parsed[`${methodName}Response`]);
        });
};
