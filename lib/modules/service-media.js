...
OnvifServiceMedia.prototype.getNodes = function() {
    let soapBody = '<tptz:GetNodes/>';

    return this.sendRequest('GetNodes', soapBody)
};

OnvifServiceMedia.prototype.getPresets = function(profileToken) {
    let soapBody = '';
    soapBody += '<tptz:GetPresets>';
    soapBody += `<tptz:ProfileToken>${profileToken}</tptz:ProfileToken>`;
    soapBody += '</tptz:GetPresets>';

    return this.sendRequest('GetPresets', soapBody)
};

OnvifServiceMedia.prototype.getPTZConfigurations = function() {
    let soapBody = '<tptz:GetConfigurations/>';

    return this.sendRequest('GetConfigurations', soapBody)
};
...
OnvifServiceMedia.prototype.validateXAddr = function (xaddr) {
 	const errorMessage = helpers.isInvalidValue(xaddr, 'string');
    if (errorMessage) {
...
OnvifServiceMedia.prototype.validateConfigurationToken = function (token) {
	const errorMessage = helpers.isInvalidValue(token, 'string');
    if (errorMessage) {
...
OnvifServiceMedia.prototype.validateProfileToken = function (token, allowEmpty) { // TODO: Allow empty should accept undefined
	const errorMessage = helpers.isInvalidValue(token, 'string', allowEmpty);
    if (errorMessage) {
...
OnvifServiceMedia.prototype.validateProfileName = function (name) {
	const errorMessage = helpers.isInvalidValue(name, 'string');
    if (errorMessage) {
...
OnvifServiceMedia.prototype.validateStreamingProtocol = function (protocol) {
	const errorMessage = helpers.isInvalidValue(protocol, 'string');
    if (errorMessage) {
        return this.setLastError(`profileName invalid: ${errorMessage}`);
    }
...
OnvifServiceMedia.prototype.validateVideoEncoderConfiguration = function (c) {
    ...
    const errorMessage = helpers.isInvalidValue(c, 'object');
    if (errorMessage) {
        return this.setLastError(`configuration invalid: ${errorMessage}`);
    }

    if (!('name' in c) || !helpers.isStringValue(c.name) || c.name.length > 64) {
        return this.setLastError('configuration.name is required, must be a string with a maximum length of 64');
    }

    if (!('encoding' in c) || !helpers.isStringValue(c.encoding) || !['JPEG', 'MPEG4', 'H264'].includes(c.encoding)) {
    ...
    if (c.encoding === 'H264') {
        if (!('h264' in c) || !helpers.isObjectValue(c.h264)) {
            return this.setLastError('configuration.h264 is required, must be an object');
        }

        if (!('govLength' in c.h264) || !helpers.isIntValue(c.h264.govLength)) {
            return this.setLastError('configuration.h264.govLength is required, must be an integer');
        }

        if (!('h264Profile' in c.h264) || !helpers.isStringValue(c.h264.h264Profile) || !['Baseline', 'Main', 'Extended', 'High'].includes(c.h264.h264Profile)) {
    ...
    if (!('multicast' in c) || !helpers.isObjectValue(c.multicast)) {
        return this.setLastError('configuration.multicast is required, must be an object');
    }

    if (!('address' in c.multicast) || !helpers.isObjectValue(c.multicast.address)) {
        return this.setLastError('configuration.multicast.address is required, must be an object');
    }

    if (!('type' in c.multicast.address) || !helpers.isStringValue(c.multicast.address.type) || !['IPv4', 'IPv6'].includes(c.multicast.address.type)) {
    ...
    if (c.multicast.address.type === 'IPv4') {
        if (!('ipv4Address' in c.multicast.address) || !helpers.isStringValue(c.multicast.address.ipv4Address)) {
...
OnvifServiceMedia.prototype.getVideoEncoderConfigurationOptions = function (params) {
    ...
    if (params) {
        if ('profileToken' in params && helpers.isStringValue(params.profileToken)) {
            soapBody += `<trt:ProfileToken>${params.profileToken}</trt:ProfileToken>`;
        }
        if ('configurationToken' in params && helpers.isStringValue(params.configurationToken)) {
...
OnvifServiceMedia.prototype.getMetadataConfiguration = function(params, callback) {
    let promise = new Promise((resolve, reject) => {
        let err_msg = '';
        if(err_msg = helpers.isInvalidValue(params, 'object')) {
            reject(new Error('The value of "params" was invalid: ' + err_msg));
            return;
        }

        if(err_msg = helpers.isInvalidValue(params['ConfigurationToken'], 'string')) {
...
OnvifServiceMedia.prototype.addMetadataConfiguration = function(params, callback) {
    let promise = new Promise((resolve, reject) => {
        ...
        if(err_msg = helpers.isInvalidValue(params['ProfileToken'], 'string')) {
...
OnvifServiceMedia.prototype.getServiceCapabilities = function() {
    const soapBody = '<trt:GetServiceCapabilities/>';

	return this.sendRequest('GetServiceCapabilities', soapBody)
		.then(result => result);
};

OnvifServiceMedia.prototype.getServiceCapabilities2 = function() {
    const soapBody = '<tr2:GetServiceCapabilities/>';
...
OnvifServiceMedia.prototype.getStreamUri2 = function(profileToken, protocol) {

    let soapBody = '';
    soapBody += '<tr2:GetStreamUri>';
    soapBody += `<tt:ReferenceToken>${profileToken}</tt:ReferenceToken>`;
    soapBody += '</tr2:GetStreamUri>';
