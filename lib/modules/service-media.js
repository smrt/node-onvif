...
OnvifServiceMedia.prototype.validateProfileToken = function (token, allowEmpty) {
    ...
    if (!token.match(/^[a-zA-Z0-9-]{12,}$/)) {
        return this.setLastError('profileToken must be of at least length 12, and only contain a-z, A-Z, 0-9 and "-"');
    }
...
OnvifServiceMedia.prototype.validateVideoEncoderConfiguration = function (params) {
    // Source: https://www.onvif.org/ver10/media/wsdl/media.wsdl 75. SetVideoEncoderConfiguration

    const { configuration: c } = params;
    ...
    return {
        ...
        extension: !('Extension' in config) ? null : {
            rotate: !('Rotate' in config['Extension']) ? null : {
                ...
                extension: config['Extension']['Rotate']['Extension']
            },
            extension: !('Extension' in config['Extension']) ? null : {
...
OnvifServiceMedia.prototype.getVideoEncoderConfigurations = function() {
    ...
    return this.sendRequest('GetVideoEncoderConfigurations', soapBody)
        .then(data => data['Configurations'].map(config => this.parseVideoEncoderConfiguration(config)));
};
...
OnvifServiceMedia.prototype.addVideoEncoderConfiguration = function(profileToken, configurationToken) {
    ...
    soapBody += `<tt:ProfileToken>${profileToken}</tt:ProfileToken>`;
    soapBody += `<tt:ConfigurationToken>${configurationToken}</tt:ConfigurationToken>`;
    soapBody += '</trt:AddVideoEncoderConfiguration>';
...
OnvifServiceMedia.prototype.getProfiles = function() {
    ...
    return this.sendRequest('GetProfiles', soapBody)
        .then(data => data['Profiles'].map(profile => this.parseProfile(profile)));
};
...
OnvifServiceMedia.prototype.getVideoSourceConfigurations = function () {
    ...
    this.sendRequest('GetVideoSourceConfigurations', soapBody)
        .then(data => data['GetVideoSourceConfigurations'].map(config => this.parseVideoSourceConfiguration(config)));
};
...
OnvifServiceMedia.prototype.getCompatibleVideoSourceConfigurations = function(profileToken) {
    ...
    this.sendRequest('GetCompatibleVideoSourceConfigurations', soapBody)
        .then(data => data['GetVideoSourceConfigurations'].map(config => this.parseVideoSourceConfiguration(config)));
};
...
OnvifServiceMedia.prototype.getSnapshotUri = function(profileToken) {
    ...
    this.sendRequest('GetSnapshotUri', soapBody)
        .then(data => this.parseStreamUri(data['MediaUri']));
};
